...
private let logger = Logger(subsystem: "com.syndicatemike.Mimic", category: "HeliusService")
...
class HeliusService {
    ...
    /// URLSession configured with timeout to prevent hanging requests
    private let session: URLSession

    private init() {
        let config = URLSessionConfiguration.default
        config.timeoutIntervalForRequest = 30 // 30 second request timeout
        config.timeoutIntervalForResource = 60 // 60 second max resource time

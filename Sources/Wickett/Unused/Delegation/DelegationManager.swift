...
class DelegationManager: ObservableObject {
    ...
    /// Get current delegation status (with caching)
    /// - Parameter force: If true, bypasses cache and fetches fresh data
    func fetchDelegationStatus(force: Bool = false) async {
        // Check cache validity (skip if we have fresh data)
        if !force,
           let lastFetch = lastFetchTime,
           delegationStatus != nil,
           Date().timeIntervalSince(lastFetch) < cacheValiditySeconds {
            logger.info("📊 Using cached delegation status (age: \(Int(Date().timeIntervalSince(lastFetch)))s)")
            return
        }
        ...
        logger.info("📊 Fetching delegation status from server...")

...
class PaymentRequestViewModel: ObservableObject {
    ...
    private let delegationManager = DelegationManager.shared
    ...
    /// Suggested recipients sorted by frequency (most interacted first)
    ...
    func createRequest() async {
        ...
        do {
            // Get portfolio from delegation, or default to 100% USDC
            let portfolio: [PortfolioAllocation]
            if let delegation = delegationManager.delegationStatus?.delegation {
                portfolio = delegation.portfolio
            } else {
                // Default to 100% USDC if no delegation set up
    ...
    func loadRecentRecipients(userId: String) async {
        await recipientsService.fetchRecentRecipients(userId: userId)
